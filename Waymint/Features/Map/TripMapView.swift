    ...
    var body: some View {
        Group {
            if stopsWithCoordinates.isEmpty {
                EmptyStateView(
                    systemImage: "map",
                    title: "Mapa zatim nema body",
                    message: "Najdi misto pres Apple Mapy pri pridani zastavky a zobrazime ho v poradi planu."
                )
            } else {
                ZStack(alignment: .bottom) {
                    ...
                    HStack(spacing: 10) {
                        Label("\(stopsWithCoordinates.count) mist", systemImage: "mappin.and.ellipse")
                        Spacer()
                        Text(routeLines.contains(where: \.isEstimated) ? "Odhad trasy" : "Apple Mapy")
                    }
                    .font(.caption.weight(.semibold))
...
enum TripMapRouteBuilder {
    static func routeLines(for trip: TripPlan) async -> [TripMapRouteLine] {
        ...
        for index in 1..<stops.count {
            ...
            if let routeLine = try? await routeLine(from: from, to: to, transportMode: segment?.transportMode ?? .walking) {
                lines.append(routeLine)
            } else {
                var coordinates = [from, to]
                lines.append(TripMapRouteLine(polyline: MKPolyline(coordinates: &coordinates, count: coordinates.count), isEstimated: true))
            }
        }
        return lines
    }
